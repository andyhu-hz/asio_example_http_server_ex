...
namespace timax
{
	namespace websocket
	{
		...
		void websocket_connection::upgrade_to_websocket(
			request const& req, reply& rep, boost::string_ref const& client_key, ws_config_t cfg)
		{
			...
			rep.set_status(reply::switching_protocols);
...
// 				startTimeout<WebSocket<isServer>::onEnd>();
		...
		timax::websocket::websocket_connection::close_frame_t websocket_connection::parse_close_payload(char *src, size_t length)
		{
			close_frame_t cf = {};
			if (length >= 2)
			{
